...
func main() {
	...
	flag.Usage = func() {
		...
		fmt.Println("  slackapi pins.add [channel] [item_id]                    Pins an item to a channel")
		fmt.Println("  slackapi pins.list [channel]                             Lists items pinned to a channel")
		fmt.Println("  slackapi pins.remove [channel] [item_id]                 Un-pins an item from a channel")
		...
		fmt.Println("  slackapi stars.add [channel] [item_id]                   Adds a star to an item")
		fmt.Println("  slackapi stars.list [count] [page]                       Lists stars for a user")
		fmt.Println("  slackapi stars.remove [channel] [item_id]                Removes a star from an item")
	...
	case "pins.add":
		PrintAndExit(client.PinsAdd(flag.Arg(1), flag.Arg(2)))

	case "pins.list":
		PrintAndExit(client.PinsList(flag.Arg(1)))

	case "pins.remove":
		PrintAndExit(client.PinsRemove(flag.Arg(1), flag.Arg(2)))
	...
	case "stars.add":
		PrintAndExit(client.StarsAdd(flag.Arg(1), flag.Arg(2)))
	...
	case "stars.remove":
		PrintAndExit(client.StarsRemove(flag.Arg(1), flag.Arg(2)))
